...
class GenAntBuildXml:
    ...
    def generate(self, setup_py_file_path='test/setup.py', simulation_subdir_path='test/modelsim/build_modelsim.xml'):
        ...
        echo.text = "STARTED"
        ex = ET.SubElement(t, "exec", executable="${basedir}/" + simulation_dir_prefix + "work/" + context['tb_top_entity'].lower(), dir=simulation_dir_prefix + "work")
        ...
        if "test_suites" in static_setup_data:
            ...
            for test_suite, test_suite_data in test_suite_data_dict.items():
                ex = ET.SubElement(prl, "exec", executable="${basedir}/" + simulation_dir_prefix + "work/" + context['tb_top_entity'].lower(), dir=simulation_dir_prefix + "work")
                ...
                if  "index" in test_suite_data:
                    ...
                    ET.SubElement(ex, "arg",  value="-gmachine_value_width=" + str((2 ** (int(test_suite_data["index"]) % 4)) * 32))

            echo = ET.SubElement(t, "echo", file="${basedir}/" + simulation_dir_prefix + "../SimulationResults/testSuitesSimulation.end", append="false")
            echo.text = "ENDED"
