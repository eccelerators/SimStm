...
class GenAntBuildXml:
    ...
    def generate(self, setup_py_file_path='test/setup.py', simulation_subdir_path='test/ghdl/build_ghdl_axi4lite.xml'):
        
        target_prefix = 'ghdl-docker-'
        ...
        root = ET.Element("project", name='ghdl')
        
        ET.SubElement(root, "property", name="docker-container-name", value="ghdl")
        ET.SubElement(root, "property", name="docker-executable", value="docker")
        ET.SubElement(root, "property", name="to-docker-path-replace", value="C:\Data")
        ET.SubElement(root, "property", name="to-docker-path-replace_by", value="/data")
        
        
        t = ET.SubElement(root, "loadresource", property="docker-basedir")
        ET.SubElement(t, "propertyresource", name="basedir")
        fc = ET.SubElement(t, "filterchain")
        tf = ET.SubElement(fc, "tokenfilter")        
        ET.SubElement(tf, "filetokenizer")  
        rs0 = ET.SubElement(tf, "replacestring")
        rs0.set("from", "${to-docker-path-replace}")
        rs0.set("to", "${to-docker-path-replace_by}")          
        rs1 = ET.SubElement(tf, "replacestring")
        rs1.set("from", "\\")
        rs1.set("to", "/")            
        ET.SubElement(root, "property", name="ghdl-docker-executable", value = "ghdl")
        ...
        ET.SubElement(
            ...
            depends=" ghdl-docker-clean, ghdl-docker-prepare, ghdl-docker-compile, ghdl-docker-elaborate, ghdl-docker-simulate-suites, ghdl-docker-exit-on-junit-errors-or-failures")
        ...
        ex = ET.SubElement(t, "exec", executable="${docker-executable}", dir=simulation_dir_prefix + "work", failonerror="true")
        ET.SubElement(ex, "arg", value="exec")
        ET.SubElement(ex, "arg", value="-t")
        ET.SubElement(ex, "arg", value="${docker-container-name}")
        ET.SubElement(ex, "arg", value="${ghdl-docker-executable}")
        ET.SubElement(ex, "arg",  value="-m")
        ET.SubElement(ex, "arg",  value="--std=08")
        ET.SubElement(ex, "arg",  value="-frelaxed")
        ET.SubElement(ex, "arg", value="--workdir=${docker-basedir}/"+ simulation_dir_prefix + "work")
        ET.SubElement(ex, "arg", value="-o")
        ET.SubElement(ex, "arg", value="${docker-basedir}/"+ simulation_dir_prefix + "work/" + context['tb_top_entity'])
        ...
        echo.text = "STARTED"
        ex = ET.SubElement(t, "exec", executable="${docker-executable}", dir=simulation_dir_prefix + "work")
        ET.SubElement(ex, "arg", value="exec")   
        ET.SubElement(ex, "arg", value="-t")
        ET.SubElement(ex, "arg", value="${docker-container-name}")
        ET.SubElement(ex, "arg", value="${docker-basedir}/" + simulation_dir_prefix + "work/" + context['tb_top_entity'])              
        ET.SubElement(ex, "arg",  value="--stop-time=100ms")
        ET.SubElement(ex, "arg",  value="--ieee-asserts=disable-at-0")
        ET.SubElement(ex, "arg",  value="-gstimulus_path=${docker-basedir}/tb/simstm/") 
        ...
        ET.SubElement(ex, "arg", value="-gstimulus_file=" + context["tb_simstm_entry_file"])
        ET.SubElement(ex, "arg", value="-gstimulus_main_entry_label=" + context["tb_simstm_entry_namespace"] + "." + "testMain")
        ET.SubElement(ex, "arg",  value="--vcd=${docker-basedir}/" + simulation_dir_prefix + "work/trace.vcd")              
        echo = ET.SubElement(t, "echo", file=simulation_dir_prefix + "work/simulation.ended", append="false")
        echo.text = "ENDED"   
        
        t = ET.SubElement(root, "target", name=target_prefix + "view-waveforms", description="view waveforms of trace.vcd")
        ex = ET.SubElement(t, "exec", executable="${gtkwave-executable}", dir=simulation_dir_prefix + "work")
        ET.SubElement(ex, "arg",  value="${basedir}/" + simulation_dir_prefix + "work/trace.vcd")
        ...
        if "test_suites" in static_setup_data:
            ...
            for test_suite, test_suite_data in test_suite_data_dict.items(): 
                ...
                ET.SubElement(ex, "arg", value="exec")
                ET.SubElement(ex, "arg", value="-t")
                ET.SubElement(ex, "arg", value="-w")
                ET.SubElement(ex, "arg", value="${docker-basedir}/" + simulation_dir_prefix + "work")
                ET.SubElement(ex, "arg", value="${docker-container-name}")
                ET.SubElement(ex, "arg", value="${docker-basedir}/" + simulation_dir_prefix + "work/" + context['tb_top_entity'])     
                ET.SubElement(ex, "arg",  value="--stop-time=100ms")
                ET.SubElement(ex, "arg",  value="--ieee-asserts=disable-at-0")
                ...
                if  "index" in test_suite_data:
                    ...
                    ET.SubElement(ex, "arg", value="-gmachine_value_width=" +
                                  str((2 ** (int(test_suite_data["index"]) % 4)) * 32))
            ...
            if shf['file_type'] == 'Verilog':
                ...
                if 'ghdl_options' in shf:
                    for o in shf['ghdl_options']:
                         ET.SubElement(ex, "arg", value=o)
                ET.SubElement(ex, "arg", value="--workdir=${docker-basedir}/"+ simulation_dir_prefix + "work")
                ET.SubElement(ex, "arg", value='${docker-basedir}/' + shf['file'])
            ET.SubElement(t, "touch", file='${docker-basedir}/' + time_stamps_prefix + shf['file'].replace('/', '_'))   

        t = ET.SubElement(root, "target", name=target_prefix + "exit-on-junit-errors-or-failures", description="exit build on junit errors or failures")
...
@click.option('--outdir_simulation_subdir', default='../../../simulation/ghdl',  help='simulation_subdir_path')
