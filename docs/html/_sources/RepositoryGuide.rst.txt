...
- ``src/tb_simstm``: The testbench top entity and architecture to be instanciated in the the user testbench.
...
- ``helper/generate-ghdl-docker-ant-build-xml.py``: Generating ``simulation/ghdl/build-ghdl.xml`` based on ``setup.py``.
...
Similar scripts are provided for modelsim.
